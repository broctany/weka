...
public class HierarchicalClusterer extends AbstractClusterer implements OptionHandler, CapabilitiesHandler, Drawable {
    ...
    class Node implements Serializable {
        ...
        public String toJsonString2(int attIndex) {
            DecimalFormat myFormatter = new DecimalFormat("#.#####");

            if (m_left == null) {
                if (m_right == null) {
                    return "[{id-" + m_instances.instance(m_iLeftInstance).value(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "}, {id-" +
                            m_instances.instance(m_iRightInstance).value(attIndex) +":" + myFormatter.format(m_fRightLength) + "}]";
                } else {
                    return "[{id-" + m_instances.instance(m_iLeftInstance).value(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "}, [" +
                            m_right.toJsonString2(attIndex) + ":" + myFormatter.format(m_fRightLength) + "]]";
                }
            } else {
                if (m_right == null) {
                    return "[[" + m_left.toJsonString2(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "], {id-" +
                            m_instances.instance(m_iRightInstance).value(attIndex) + ":" + myFormatter.format(m_fRightLength) + "}]";
                } else {
                    return "[[" + m_left.toJsonString2(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "], [" +m_right.toJsonString2(attIndex) + ":" + myFormatter.format(m_fRightLength) + "]]";
                }
            }
        }
    ...
    public String jsonGraph() throws Exception {
        if (numberOfClusters() == 0) {
            return "[]";
        }
        int attIndex = m_instances.classIndex();
